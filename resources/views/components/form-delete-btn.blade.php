@props(['action', 'text' => 'Delete', 'color' => 'red', 'confirmMessage' => 'Are you sure you want to delete this item?'])
@php
  $colorClasses = [
...
<form method="POST" action="{{ $action }}" {{ $attributes->merge(['class' => 'inline-flex']) }}
  onsubmit="return confirm('{{ $confirmMessage }}')">
  @csrf
  @method('DELETE')
  <button type="submit"
    class="transition-all duration-300 cursor-pointer rounded-md px-3 py-2 text-sm font-semibold text-white shadow-xs focus-visible:outline-2 focus-visible:outline-offset-2 {{ $classes }}">
    {{ $text }}
  </button>
</form>